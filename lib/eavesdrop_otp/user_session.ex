defmodule EavesdropOTP.UserSession do
  ...
  def last_message(user_name) do
    %UserState{last_message: last_message, current_track: _, user_name: _} = status(user_name)
    last_message
  end

  @doc "Get the current track name for a user"
  def current_track(user_name) do
    %UserState{current_track: current_track, last_message: _, current_track: _} = status(user_name)
    current_track
  end

  @doc "utility function to get the current state held in the statem process"
  defp status(user_name) do
    {:ok, state} = :gen_statem.call(via_tuple(user_name), :status)
    state
  end
  ...
  def idle({:call, from}, :idle, %UserState{user_name: user_name} = state) do
    {:next_state, :play, state, [{:reply, from, :play}]}
  end
  def idle({:call, from}, {:play, track}, %UserState{user_name: user_name} = state) do
    ...
    {:next_state, :play, %UserState{state| last_message: message, current_track: track}, [{:reply, from, {:ok, message}}]}
  end
  ...
  def idle(_any, from, %UserState{user_name: user_name} = state) do
    ...
    {:keep_state, state, [{:reply, from, {:ok, message}}]}
  end
  ...
  def play({:call, from}, :stop, %UserState{user_name: user_name} = state) do
    ...
    {:next_state, :idle, %UserState{state| last_message: message, current_track: nil}, [{:reply, from, {:ok, message}}]}
  end
  ...
  def play(_any, from, %UserState{user_name: user_name} = state) do
    ...
    {:keep_state, %UserState{state| last_message: message, current_track: nil}, [{:reply, from, {:ok, message}}]}
  end
end
